...
export default function ChatInput({
  ...
  const available_users = user_list.filter(
    (user) => user.toLowerCase() !== current_nickname.toLowerCase()
  );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        emoji_picker_ref.current &&
        !emoji_picker_ref.current.contains(event.target as Node)
      ) {
        setShowEmojiPicker(false);
      }
      if (
        whisper_menu_ref.current &&
        !whisper_menu_ref.current.contains(event.target as Node)
      ) {
        setShowWhisperMenu(false);
      }
      if (
        ai_modal_ref.current &&
        !ai_modal_ref.current.contains(event.target as Node)
      ) {
        if (!is_ai_loading) {
  ...
  const handleAISubmit = async () => {
    ...
    try {
      const chat_context =
        messages_for_ai
          ?.map((msg) => `${msg.nickname}: ${msg.message || "[미디어]"}`)
          .join("\n") || "";
      ...
      const ai_response =
        data.choices?.[0]?.message?.content || "AI 응답을 받지 못했습니다.";
    ...
    >
      ...
      <div className="flex items-center gap-2 max-w-2xl w-full">
        ...
        <div className="flex items-center gap-2 relative">
          ...
          <label
            htmlFor="file-upload"
            className={`neumorphic-button w-8 h-8 rounded-full flex items-center justify-center cursor-pointer transition-all ${
          ...
          {available_users.length > 0 && (
            <div className="relative" ref={whisper_menu_ref}>
              <button
                ...
                disabled={disabled}
                className={`neumorphic-button w-8 h-8 rounded-full flex items-center justify-center transition-all disabled:cursor-not-allowed ${
                  selected_target ? "opacity-100" : "opacity-70"
                }`}
                style={{
      ...
      {is_uploading && (
        <div
          className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-full text-sm mb-2"
          style={{
