      ...
      if (parsed_data.type === "join") {
        ...
        const is_duplicate = Array.from(clients.entries()).some(
          ([existing_ws, existing_nickname]) =>
            existing_ws !== ws &&
            existing_nickname.toLowerCase() === trimmed_nickname.toLowerCase()
        );
        ...
        const message_id = `${Date.now()}-${nickname}-${Math.random()
          .toString(36)
          .substring(7)}`;
        ...
        if (parsed_data.file_data) {
          ...
          console.log(
            `[서버] 파일 메시지 수신: ${parsed_data.file_name} (${parsed_data.file_size} bytes, base64: ${file_data_size} bytes)`
          );
        }
        ...
        if (readers && !readers.has(reader_nickname)) {
          ...
          const total_users = getRealUserCount();
          const unread_count =
            total_users > 1 ? Math.max(0, total_users - 1 - readers.size) : 0;
        ...
        if (parsed_data.file_data) {
          const file_data_size = parsed_data.file_data.length;
          console.log(
            `[서버] 귓속말 파일 수신: ${parsed_data.file_name} (${parsed_data.file_size} bytes, base64: ${file_data_size} bytes)`
          );
        }

        sendWhisper(whisper, ws);
        console.log(
          `${sender_nickname} -> ${target_nickname}: ${
            whisper_message ||
            parsed_data.emoji ||
        ...
        if (typeof new_time === "number" && new_time >= 1 && new_time <= 60) {
          ...
          console.log(
            `[삭제시간] ${nickname}이 ${delete_time_minutes}분으로 변경`
          );
        }
        ...
        const is_duplicate = Array.from(clients.entries()).some(
          ([existing_ws, existing_nickname]) =>
            existing_ws !== ws &&
            existing_nickname.toLowerCase() === new_nickname.toLowerCase()
        );
        ...
        const message_id = `ai-${Date.now()}-${Math.random()
          .toString(36)
          .substring(7)}`;
        const total_users = getRealUserCount();
        const unread_count = total_users > 1 ? total_users - 1 : 0;
...
function sendWhisper(whisper: ChatMessage, sender_ws: WebSocket) {
  ...
  clients.forEach((nickname, client) => {
    if (client.readyState === WebSocket.OPEN) {
      ...
      const is_target =
        nickname.toLowerCase() === target_nickname.toLowerCase();
